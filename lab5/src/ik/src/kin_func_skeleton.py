...
def rotation_3d(omega, theta):
    ...
    # YOUR CODE HERE
    return np.eye(3) + skew_3d(omega)/np.linalg.norm(omega)*math.sin(np.linalg.norm(omega)*theta) + np.dot(skew_3d(omega), skew_3d(omega))/(np.linalg.norm(omega))**2*(1-math.cos(np.linalg.norm(omega)*theta))
...
def homog_3d(xi, theta):
    ...
    if (np.linalg.norm(w)==0):
        ...
        p = (np.dot((np.eye(3) - R), np.matmul(skew_3d(w),v)) + np.dot(np.matmul(np.reshape(w, (3,1)),np.reshape(w, (1,3))), (v*theta)))/np.linalg.norm(w)**2

    A[0:3, 0:3] = R
    A[0:3, 3:4] = np.reshape(p, (3,1))
    A[3][3] = 1
